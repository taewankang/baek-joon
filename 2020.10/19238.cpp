...
Customer pushCustomer(int py, int px, int dy, int dx, int val, int cnt) {
    Customer customer = {py, px, dy, dx, val, cnt};
    return customer;
}
...
int findCustomerDistance() {
    ...
    for(int i = 0; i < customer.size(); i++) {
        if(customer[i].py != -1 && customer[i].px != -1) //작업이 끝난 손님이 아니라면
            pq.push(pushCustomer(customer[i].py, customer[i].px, customer[i].dy, customer[i].dx, vec[customer[i].py][customer[i].px], customer[i].cnt));
    }
...
void goDestination(int idx) {
    ...
    taxi.x = customer[idx].dx;
    taxi.fuel -= dist;
    customer[idx] = pushCustomer(-1, -1, -1, -1, 0, 0);
    if(taxi.fuel < 0) except();
    taxi.fuel += dist * 2;
}
...
int main(void){
    ...
    for(int i = 0; i < M; i++) {
        cin >> a >> b >> c >> d;
        customer.push_back(pushCustomer(a - 1, b - 1, c - 1, d - 1, 0, i));
    }
    while(1) {
        int customerIdx = findCustomerDistance();
        if(customerIdx == -1) break; 
